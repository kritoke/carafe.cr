...
Configuration File:
  A .safe-run.conf file in the project root can set these options:
...
# Kill existing processes for the project
cleanup_processes() {
...
# Start the application with Anti-Freeze pattern
start_app() {
  # Elm doesn't have a server process - it runs in the browser
