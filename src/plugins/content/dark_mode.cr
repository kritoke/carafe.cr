...
module Carafe::Plugins::DarkMode
  class Generator
...
class Carafe::Plugins::DarkModePlugin < Carafe::Plugin
  def name : String
    "dark_mode"
  end
  ...
  def enabled?(config : Carafe::Config) : Bool
    ...
    return false if dark_mode_enabled.nil?

    dark_mode_enabled.as_bool? || (dark_mode_enabled.as_s? == "true")
  end

  def register(site : Carafe::Site) : Nil
    puts "DarkModePlugin: Registering dark mode plugin" unless site.config.quiet?
  end
end

Carafe::Plugin.register_implementation(Carafe::Plugins::DarkModePlugin)
