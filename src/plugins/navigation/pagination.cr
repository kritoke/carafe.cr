...
class Carafe::Plugins::PaginationPlugin < Carafe::Plugin
  def name : String
    "pagination"
  end
  ...
  class Generator < Carafe::Generator
    ...
    def paginate_resource(resource : Carafe::Resource, config : YAML::Any, paginator_resources : Array(Carafe::Resource))
      ...
      chunks.each_with_index do |chunk_items, i|
        paginator = Carafe::Paginator.new(chunk_items, i + 1, pages, per_page)
        if i.zero?
...
Carafe::Plugin.register_implementation(Carafe::Plugins::PaginationPlugin)
